## Unit tests for ShipInterior module placement zones. Verifies 3-zone layout,
...
func register_tests() -> void:
	# Zone count
	add_test("zone_count_is_three", _test_zone_count_is_three)
	# Initial state
...
func _test_zone_count_is_three() -> void:
	assert_equal(_interior.get_zone_count(), 3, "Ship interior should have 3 placement zones")
...
func _test_place_module_positions_at_zone_center() -> void:
	...
	assert_equal(mock_a.position, Vector3(-3.0, 0.0, -1.0),
		"Zone 0 module should be at ZONE_A_CENTER")
	assert_equal(mock_b.position, Vector3(0.0, 0.0, -1.0),
		"Zone 1 module should be at ZONE_B_CENTER")
	assert_equal(mock_c.position, Vector3(3.0, 0.0, -1.0),
...
func _test_first_empty_zone_skips_occupied() -> void:
	...
	assert_equal(_interior.get_first_empty_zone(), 2,
		"First empty zone should be 2 when zones 0 and 1 are occupied")
...
func _test_first_empty_zone_returns_neg1_when_all_full() -> void:
	...
	assert_equal(_interior.get_first_empty_zone(), -1,
		"First empty zone should be -1 when all 3 zones are full")
...
func _test_is_zone_occupied_invalid_index_returns_false() -> void:
	...
	assert_false(_interior.is_zone_occupied(3), "Index 3 (out of range) should return false")
...
func _test_get_module_at_invalid_index_returns_null() -> void:
	...
	assert_null(_interior.get_module_at_zone(3), "Index 3 should return null")
