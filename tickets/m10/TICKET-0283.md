...
- [ ] Remove all `budget_usd` fields from agent entries
- [ ] Add a `max_turns` integer field to each agent entry (or a top-level `default_max_turns`
      with per-agent overrides)
- [ ] Suggested defaults: gameplay-programmer `150`, qa-engineer `100`, other agents `75`
      (tune as appropriate — these replace the current hardcoded `200`)

### `orchestrator/conductor.py`
- [ ] Remove `get_budget()` function and all call sites
- [ ] Remove the `if budget > 0` conditional that gates `--max-turns`
- [ ] Always pass `--max-turns <agent_max_turns>` when invoking the Claude CLI
- [ ] Read `max_turns` from the agent config entry (fall back to `default_max_turns` if
      per-agent value is absent)

### No Regressions
- [ ] All existing agent dispatch paths still receive a `--max-turns` argument
- [ ] No orphaned references to `budget_usd` or `get_budget` remain in the codebase
